...
public class CriaMarcaDagua {
	...
	public void criaMarcaDaguaFile(String text, Integer linha, Integer rotation, String src, JTextArea textArea) throws IOException, DocumentException {
		...
		for (int i = 1; i <= n; i++) {
			...
			for (int w = 1;w < pagesize.getRight() * vertical + textoAserImpresso.length() * fontsize  ; w = w + textoAserImpresso.length() * fontsize ) {
                textoAserImpresso += " " + textoAserImpresso ;
            }
			for (int j = 1; j <=  pagesize.getTop() + vertical; j = (int) (j + (fontsize * linha) * (1 + Math.sin(rotation)))) {
                y = j;
